...
/**
 * The parameter's processor is associated with {@link SessionParameter}
 * annotation and supports building process of arguments with this annotation.
 * <p>
 *
 * for example: <pre class="prettyprint">
 * {@literal @}Path(path="/loggeduserinfo") public void
 * getInfoAboutLoggedUser({@literal @}SessionParameter("loggeduser") Customer
 * customer) { ... }
 * </pre>
 *
 *
 * In this case, processor build the customer's argument and fills it with the
 * value from the session with the name 'loggeduser'. If this attribute doesn't
 * exist in the session, then is filled by null. In case when in 'loggeduser'
 * session attribute is object which is not instance of customer, then exception
 * is throwed.
 *
 */
