...
/**
 * <i>MVC module</i> for GUICE. <p>
 *
 * If you are fammiliar with the GUICE servlets, then usage of the Lime MVC is
 * pretty straight forward. MvcModule is basically extended version of Guice's
 * ServletModule and you can use all ServletModule's methods in
 * configureControllers() method implementation (like serve() etc..). <p>
 *
 * In your web application, put new MvcModule with implemented
 * configureControllers() method into GuiceServletContextListener
 * implementation.
 *
 * <p>example: <pre class="prettyprint"> public class WebAppConfiguration
 * extends GuiceServletContextListener { ... protected Injector getInjector() {
 * Injector injector = Guice.createInjector( new MvcModule() { protected void
 * configureControllers() {
 *
 * control("/someController/*") .withController(SomeController.class) ... } } );
 * return injector; } }
 * </pre> Example shows the basic usage and registers the simple controller
 * class. All requests starting with '/someController/' will be processed by the
 * SomeController.
 *
 * <p> It is possible to have more controllers for one URL registration and then
 * all controllers will be invoked. This is good when you have diplayed several
 * independent informations in the view.
 *
 * <p>example: <pre class="prettyprint"> public class WebAppConfiguration
 * extends GuiceServletContextListener { ... protected Injector getInjector() {
 * Injector injector = Guice.createInjector( new MvcModule() { protected void
 * configureControllers() {
 *
 * control("/someControllers/*") .withController(SomeController.class)
 * .withController(AnotherController.class); ... } } ); return injector; } }
 * </pre>
 *
 * In both controllers can be defined the same path. Let's assume 2 controllers:
 * <p> <pre class="prettyprint"> {@literal @}Controller
 * {@literal @}View("some_view") public class SomeController { ...
 * {@literal @}Path("/get") {@literal}Model("user") public User getUser() { ...
 * } }
 *
 * {@literal @}Controller {@literal @}View("some_view") public class
 * AnotherController { ... {@literal @}Path("/get") {@literal}Model("goods")
 * public List<Product> getGoods() { ... } }
 * </pre>
 *
 * In that case, both methods 'getUser' and 'getGoods' will be invoked. Be
 * carrefull when you're invoking 2 or more methods. It may cause problems with
 * multiple views when only first view is choosen. <p>
 *
 * @see Controller
 * @see ModelMap
 * @see ModelAndView
...
// ------------------------------------------------------------------------
    ...
    /**
     * The method register into {@link ViewScannerService} a custom view scanner
     * as a class
     *
     * @see ViewScannerService
     */
    ...
    /**
     * The method registers a custom parameter processor. The parameter
     * processors converts/prepares/fills the values into invoked method's
     * parameters. All predefined processors are placed in 'parameters'
     * sub-package.
     *
    ...
    /**
     * Method registers a global interceptor class as a singleton. These global
     * interceptors do pre/post processing for every request/response.
     *
     *
    ...
    /**
     * Method registers a global interceptor instance. These global interceptors
     * do pre/post processing for every request/response.
     *
     * @param interceptorHandler
     */
