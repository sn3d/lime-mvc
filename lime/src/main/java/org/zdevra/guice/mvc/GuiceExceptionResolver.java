...
/**
 * Lime's Exception resolver implementation which resolves all throwed
 * exceptions from controllers.
 *
 * For all exceptions is used generally {@link DefaultExceptionHandler}. If you
 * want to handle the exception, just make binding between exception and your
 * handler.
 *
 * Example of custom exception handling. <pre class="prettypring"> public void
 ...
 * </pre>
 *
 * Exception handling is executed simmilar as catching block. This means if
 * Custom2Exception in the example will be derived from Custom1Exception, the
 * resolver uses for Custom2 a first registered handler. In this case
 * Custom1ExceptionHandler.
 *
 */
@Singleton
