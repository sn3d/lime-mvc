...
/**
 * Class is representing bind/relation between an exception class and exception
 * handler. Binding are used by {@link DefaultExceptionResolver}.
 *
 * Bindings are constructed via static factory methods toClass() and
 * toInstance(). There are 2 implementations of the binding.
 * {@link ExceptionBindToClass} and {@link ExceptionBindToInstance}
 *
 *
...
abstract class ExceptionBind implements Comparable<ExceptionBind> {
     ...
     * @param handlerClass
     * @param exteption
     * @param order exception handling is executed in order. Handlers are first
     * ordered in ascending order by this order index.
     *
    ...
    /**
     * Method constructs binding for exception to concrete instance of handler.
     *
     * @param handler
     * @param exteption
     * @param order exception handling is executed in order. Handlers are first
    ...
    /**
     * Constructor is hidden, binding is constructed via factory static methods.
     *
    ...
    /**
     * Method return instance of exception handler. This depends on child's
     * implementation.
     *
     * @param injetor
