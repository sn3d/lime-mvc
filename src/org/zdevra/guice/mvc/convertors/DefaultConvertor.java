...
/*----------------------------------------------------------------------*/
	...
	public static class Factory implements ConvertorFactory {		
		...
		public Convertor createConvertor(Class<?> type, Annotation[] annotations) {
					...
					if (params != null) {
						if (params.length == 1) {
							if (params[0] == String.class) {								
								return new DefaultConvertor(type, factory);
							}
						}
					}
				}
			...
			return new DefaultConvertor(type);
		}		
	}
		
	
	private DefaultConvertor(Class<?> type) {
		this(type, null);
	}
	
	
	private DefaultConvertor(Class<?> type, Method factoryMethod) {
		this.type = type;
